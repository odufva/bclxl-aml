FUN_STATISTICS=function(genelist, data, logicalVectors, logicalVector_normals, ALTERNATIVE="greater", adj.method="BH", CORES=1, prettynum=T){
  library(parallel)
  data=do.call(rbind, mclapply(genelist, TestGeneWilcox, data, logicalVectors, logicalVector_normals=NULL, ALTERNATIVE, prettynum, mc.cores = CORES))
...
TestGeneWilcox=function(gene, data, logicalVectors, logicalVector_normals, ALTERNATIVE="greater"){
  if(!gene%in%rownames(data)) return(NULL)
  ...
  stats=do.call(rbind, lapply(seq(d_list), function(i, d_list, d_list2, ALTERNATIVE){
    l=d_list[[i]]
    name1=names(d_list)[i]
    name2=names(d_list2)

    P.value=sapply(seq(d_list2), function(j, l, d_list2, ALTERNATIVE){
      l2=d_list2[[j]]
      signif(wilcox.test(l, l2, alternative = ALTERNATIVE)$p.value,2)
    }, l, d_list2, ALTERNATIVE)

    FC=sapply(d_list2, function(l2, l1){
      2^(mean(l)-mean(l2))
    })

    data=data.frame(gene, FC, P.value, name1, name2, stringsAsFactors=F)
    rownames(data)=NULL
    data=data[!data$name1==data$name2,]

    return(data)
  },d_list, d_list2, ALTERNATIVE))
...
fun.cox.elasticnet=function(DATA_ORG, time, status, cores=8, summary.km="85th_15th_percentile", percentage=0.25, REPEATS=100){
  library(glmnet)
  ...
  if(percentage==0){
    ...
    enet.rep=do.call(rbind, parallel::mclapply(seq(REPEATS), function(r){
      ...
      s <- do.call(rbind, lapply(a, function(i){
        cv <- cv.glmnet(x=x,y=y, family = "cox", nfold = floor((dim(DATA_ORG)[1]/15)), type.measure = "deviance", alpha = i, standardize=F)
